...
export default function AutomationPage() {
  ...
  const canManageAutomation = authStatus === 'loggedIn' && hasPermission(user, 'automation', 'manage');
  ...
  const renderTriggerList = (eventTriggers: WorkflowTrigger[]) => {
    if (!eventTriggers || eventTriggers.length === 0) { // Safety check
  ...
  const renderStatusGroup = (group: WorkflowStatusGroup, title: string) => {
    // Safety check: ensure statuses is an array before filtering
    const groupStatuses = Array.isArray(statuses) ? statuses.filter(s => s.group === group) : [];
        ...
        {loadingData ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {groupStatuses.map((status) => {
               // Safety check for triggers
               const triggerCount = Array.isArray(triggers) ? triggers.filter(t => t.status_id === status.id).length : 0;
               return (
