...
export default function CustomersPage() {
  ...
  const handleEditCustomer = (customerId: string) => {
    ...
    const handleViewLocations = (customerId: string) => {
                  ...
                  filteredCustomers.map((cust) => (
                      <TableRow key={cust.id}>
                        ...
                        <TableCell className="text-right space-x-1 whitespace-nowrap">
                           <Button variant="outline" size="icon" onClick={() => handleViewLocations(cust.id)} title="View Locations" disabled={deleteMutation.isPending && deleteMutation.variables === cust.id}>
                              <MapPin className="h-4 w-4" />
                              <span className="sr-only">View Locations</span>
                           </Button>
                          {canEdit && (
                            <Button variant="outline" size="icon" onClick={() => handleEditCustomer(cust.id)} title="Edit Customer" disabled={deleteMutation.isPending && deleteMutation.variables === cust.id}>
                          ...
                          {canDelete && (
                            <AlertDialog>
                              <AlertDialogTrigger asChild>
                                <Button variant="destructive" size="icon" title="Delete Customer" disabled={deleteMutation.isPending && deleteMutation.variables === cust.id}>
