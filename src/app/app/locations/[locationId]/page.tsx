...
import { Loader2, Save, Users, AlertCircle, MapPin as MapPinIcon, Briefcase, Wrench, PlusCircle } from 'lucide-react';
      ...
      <Card>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <Controller name="name" control={control} render={({ field }) => (<div><Label>Location Name</Label><Input {...field} /></div>)} />
              <div className="md:col-span-2"><Controller name="address_line1" control={control} render={({ field }) => (<div><Label>Address Line 1</Label><Input {...field} /></div>)} /></div>
              <div className="md:col-span-2"><Controller name="address_line2" control={control} render={({ field }) => (<div><Label>Address Line 2</Label><Input {...field} value={field.value ?? ''} /></div>)} /></div>
              <Controller name="city" control={control} render={({ field }) => (<div><Label>City</Label><Input {...field} /></div>)} />
              <Controller name="province" control={control} render={({ field }) => (<div><Label>Province/State</Label><Input {...field} /></div>)} />
              <Controller name="postal_code" control={control} render={({ field }) => (<div><Label>Postal Code</Label><Input {...field} /></div>)} />
              <Controller name="country" control={control} render={({ field }) => (<div><Label>Country</Label><Input {...field} /></div>)} />
              <Controller name="location_type" control={control} render={({ field }) => (<div><Label>Location Type</Label><Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{locationTypes.map(type => <SelectItem key={type} value={type}>{formatEnum(type)}</SelectItem>)}</SelectContent></Select></div>)} />
          </CardContent>
